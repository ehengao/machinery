...
var (
	typesMap = map[string]reflect.Type{
		...
		"[]string":            reflect.TypeOf([]string{""}),
		"[]*tasks.TaskResult": reflect.TypeOf([]*TaskResult{}),
	}
...
func reflectValues(valueType string, value interface{}) (reflect.Value, error) {
	...
	// Strings
	if theType.String() == "[]*tasks.TaskResult" {
		trs := reflect.ValueOf(value)
		return trs, nil
	}
...
func getTaskResultValue(theType string, value interface{}) (*TaskResult, error) {
	s, ok := value.(*TaskResult)
	if !ok {
		return nil, typeConversionError(value, typesMap[theType].String())
	}
	return s, nil
}
